...
func TestCreateAndLoadSnapshots(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Equal(t, uint64(2), block1.HeaderBody.Height)

	block2, err := blockchain2.GetLatestBlock()
	require.NoError(t, err)
	assert.Equal(t, uint64(2), block2.HeaderBody.Height)
	...
	block1, err = blockchain1.GetLatestBlock()
	require.NoError(t, err)
	assert.Equal(t, uint64(0), block1.HeaderBody.Height)

	block2, err = blockchain2.GetLatestBlock()
	require.NoError(t, err)
	assert.Equal(t, uint64(0), block2.HeaderBody.Height)
}

...
// 	fmt.Printf("lookup: %s\n", hex.EncodeToString(in.Registers[0]))
...
func Test_SimulatedMainnetProgressesBlocks(t *testing.T) {
	...
	t.Run("GetLatestBlockHeader returns the latest sealed block from archive node", func(t *testing.T) {
		...
		accessClient := internalmock.NewMockAccessAPIClient(mockCtrl)
		executionClient := internalmock.NewMockExecutionDataAPIClient(mockCtrl)
		addGetLatestBlockHeaderCall(accessClient, expectedHeight)
		addGetBlockHeaderCall(t, accessClient, expectedHeight)

		b, _ := getBlockchain(t, expectedHeight, WithClient(executionClient, accessClient))
	...
	t.Run("GetLatestBlockHeader returns the start block", func(t *testing.T) {
		...
		accessClient := internalmock.NewMockAccessAPIClient(mockCtrl)
		executionClient := internalmock.NewMockExecutionDataAPIClient(mockCtrl)
		addGetBlockHeaderCall(t, accessClient, startHeight)

		b, _ := getBlockchain(t, expectedHeight, WithClient(executionClient, accessClient), WithStartBlockHeight(startHeight))
	...
	t.Run("GetBlockHeaderByHeight calls archive node for past blocks", func(t *testing.T) {
		...
		addGetBlockHeaderCall(t, accessClient, expectedHeight-1)
	...
	t.Run("Starting with persistent store handles blocks", func(t *testing.T) {
		...
		opts := []Option{WithClient(executionClient, accessClient), WithStartBlockHeight(startHeight)}
...
func addGetLatestBlockHeaderCall(client *internalmock.MockAccessAPIClient, height uint64) {
	responseGetLast := &access.BlockHeaderResponse{
	...
	client.EXPECT().
		GetLatestBlockHeader(context.Background(), &access.GetLatestBlockHeaderRequest{IsSealed: true}).
		Return(responseGetLast, nil).
		Times(1)
}

func addGetBlockHeaderCall(t *testing.T, client *internalmock.MockAccessAPIClient, height uint64) {
	...
	response := &access.BlockHeaderResponse{
		Block: &entities.BlockHeader{
			Height:  height,
			ChainId: flowgo.Mainnet.String(),
		},
	}

	client.EXPECT().
		GetBlockHeaderByHeight(context.Background(), request).
		Return(response, nil).
		Times(1)
}

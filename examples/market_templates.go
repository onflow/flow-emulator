...
func GenerateCreateSaleScript(tokenAddr flow.Address, marketAddr flow.Address, cutPercentage float64) []byte {
	...
	template := `
		...
		transaction {
			prepare(acct: AuthAccount) {
				let ownerVault = acct.getCapability(/public/flowTokenReceiver)!
					.borrow<&{FungibleToken.Receiver}>()
					?? panic("Couldn't borrow receiver reference from public Vault")

				let collection <- Market.createSaleCollection(ownerVault: ownerVault, cutPercentage: %f)
	...
	return []byte(fmt.Sprintf(template, tokenAddr, marketAddr, cutPercentage))
}
